...
if (!class_exists('Webflow_APIv2')) {
    ...
    class Webflow_APIv2
    {
        ...
        {
            return Webflow_API::get_instance()->remove_wf_token();
        }
        ...
        public function get_site_data($site)
        {
            ...
            $collections = $this->list_collections($site->id);
        ...
        {
            return Webflow_API::get_instance()->set_api_token($token);
        }
       ...
       /**
        ...
        public function get_site_domain($site)
        {
            ...
            $domains_response = $this->get_site_domains($site->id);
            if (!is_wp_error($domains_response)) {
                if (count($domains_response) > 0) {
                    $this->domain = $domains_response[0]->url;
        ...
        public function list_sites()
        {
            ...
            $response = wp_remote_get("https://api.webflow.com/v2/sites", array(
                "headers" => $this->get_webflow_headers(),
            ));

            $res = $this->handle_remote_response($response, new WP_Error('list_sites', __("The API key you used is invalid: failed to list sites", WEBFLOW_PAGES_TEXT_DOMAIN)));
        
            return $this->unwrap_remote_response($res, "sites");
        }
        ...
        public function get_site_domains($site_id)
        {
            ...
            $response = wp_remote_get(esc_url_raw("https://api.webflow.com/v2/sites/$site_id/custom_domains"), array(
                "headers" => $this->get_webflow_headers(),
            ));

            $res = $this->handle_remote_response($response, new WP_Error('site_domains', __("The API key you used is invalid: failed to list your site domains", WEBFLOW_PAGES_TEXT_DOMAIN)));
        
            return $this->unwrap_remote_response($res, "customDomains");
        }
        ...
        public function list_collections($site_id)
        {
            ...
            $response = wp_remote_get(esc_url_raw("https://api.webflow.com/v2/sites/$site_id/collections"), array(
                "headers" => $this->get_webflow_headers(),
            ));

            $res = $this->handle_remote_response($response, new WP_Error('list_collections', __("The API key you used is invalid: failed to list your CMS Collections", WEBFLOW_PAGES_TEXT_DOMAIN)));
        
            return $this->unwrap_remote_response($res, "collections");
        }
        ...
        {
            if (is_wp_error($response) || !isset($response['response']) || !isset($response['response']['code'])) {
                return $wp_error;
            }
            ...
            if (200 == $response_code) {
                ...
                try {
                    ...
                    return new WP_Error($error->code, property_exists($error, "msg") ? $error->msg : $error->message);
                } catch (Exception $e) {
        ...
        private function unwrap_remote_response($remote_response, $path) {
            if (is_wp_error($remote_response)) {
                return $remote_response;
            }
            $array = get_object_vars($remote_response);
            return $array[$path];
        }
    }
